...
struct _FwupdJcatMldsa87Engine {
	FuJcatEngine parent_instance;
	gnutls_x509_trust_list_t tl;
};

G_DEFINE_TYPE(FwupdJcatMldsa87Engine, fu_jcat_mldsa87_engine, FU_TYPE_JCAT_ENGINE)

static gboolean
fu_jcat_mldsa87_engine_add_pubkey_blob_fmt(FwupdJcatMldsa87Engine *self,
					   GBytes *blob,
...
static gboolean
fu_jcat_mldsa87_engine_add_public_key_raw(FuJcatEngine *engine, GBytes *blob, GError **error)
{
	FwupdJcatMldsa87Engine *self = FWUPD_JCAT_MLDSA87_ENGINE(engine);
	return fu_jcat_mldsa87_engine_add_pubkey_blob_fmt(self, blob, GNUTLS_X509_FMT_PEM, error);
}

static gboolean
fu_jcat_mldsa87_engine_add_public_key(FuJcatEngine *engine, const gchar *filename, GError **error)
{
	FwupdJcatMldsa87Engine *self = FWUPD_JCAT_MLDSA87_ENGINE(engine);
	...
	if (g_str_has_suffix(filename, ".pem")) {
		...
		if (blob == NULL)
			return FALSE;
		if (!fu_jcat_mldsa87_engine_add_pubkey_blob_fmt(self,
								blob,
								GNUTLS_X509_FMT_PEM,
								error))
			return FALSE;
...
static gboolean
fu_jcat_mldsa87_engine_setup(FuJcatEngine *engine, GError **error)
{
	FwupdJcatMldsa87Engine *self = FWUPD_JCAT_MLDSA87_ENGINE(engine);
	int rc;
...
static FuJcatResult *
fu_jcat_mldsa87_engine_verify(FuJcatEngine *engine,
			      GBytes *blob,
 ...
 *  `certtool --p7-verify --load-certificate client.pem --infile=test.p7b` */
static FuJcatResult *
fu_jcat_mldsa87_engine_self_verify(FuJcatEngine *engine,
				   GBytes *blob,
				   GBytes *blob_signature,
				   FuJcatVerifyFlags flags,
				   GError **error)
{
	...
	return fu_jcat_mldsa87_engine_verify(engine, blob, blob_signature, crt, flags, error);
}

/* verifies a detached signature just like:
 *  `certtool --p7-verify --load-certificate client.pem --infile=test.p7b` */
static FuJcatResult *
fu_jcat_mldsa87_engine_pubkey_verify(FuJcatEngine *engine,
				     GBytes *blob,
				     GBytes *blob_signature,
				     FuJcatVerifyFlags flags,
				     GError **error)
{
	return fu_jcat_mldsa87_engine_verify(engine, blob, blob_signature, NULL, flags, error);
}

static FwupdJcatBlob *
fu_jcat_mldsa87_engine_pubkey_sign(FuJcatEngine *engine,
				   GBytes *blob,
				   GBytes *cert,
				   GBytes *privkey,
...
static FwupdJcatBlob *
fu_jcat_mldsa87_engine_self_sign(FuJcatEngine *engine,
				 GBytes *blob,
				 FuJcatSignFlags flags,
				 GError **error)
{
	...
	/* sign */
	return fu_jcat_mldsa87_engine_pubkey_sign(engine, blob, cert, privkey, flags, error);
}

static void
fu_jcat_mldsa87_engine_finalize(GObject *object)
{
	FwupdJcatMldsa87Engine *self = FWUPD_JCAT_MLDSA87_ENGINE(object);
	gnutls_x509_trust_list_deinit(self->tl, 1);
	G_OBJECT_CLASS(fu_jcat_mldsa87_engine_parent_class)->finalize(object);
}

static void
fu_jcat_mldsa87_engine_class_init(FwupdJcatMldsa87EngineClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuJcatEngineClass *klass_app = FWUPD_JCAT_ENGINE_CLASS(klass);
	klass_app->setup = fu_jcat_mldsa87_engine_setup;
	klass_app->add_public_key = fu_jcat_mldsa87_engine_add_public_key;
	klass_app->add_public_key_raw = fu_jcat_mldsa87_engine_add_public_key_raw;
	klass_app->pubkey_verify = fu_jcat_mldsa87_engine_pubkey_verify;
	klass_app->pubkey_sign = fu_jcat_mldsa87_engine_pubkey_sign;
	klass_app->self_verify = fu_jcat_mldsa87_engine_self_verify;
	klass_app->self_sign = fu_jcat_mldsa87_engine_self_sign;
	object_class->finalize = fu_jcat_mldsa87_engine_finalize;
}

static void
fu_jcat_mldsa87_engine_init(FwupdJcatMldsa87Engine *self)
{
}

FuJcatEngine *
fu_jcat_mldsa87_engine_new(FuJcatContext *context)
{
	g_return_val_if_fail(FWUPD_JCAT_IS_CONTEXT(context), NULL);
	return FWUPD_JCAT_ENGINE(
	    g_object_new(FWUPD_JCAT_TYPE_MLDSA87_ENGINE,
			 "context",
			 context,
			 "kind",
			 FWUPD_JCAT_BLOB_KIND_PKCS7_MLDSA87,
			 //					"flags",
			 //					FWUPD_JCAT_ENGINE_PQ_SAFE,
			 "method",
