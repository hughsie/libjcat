...
jcat_file_add_string(JcatFile *self, guint idt, GString *str)
{
	...
	if (priv->version_major > 0 || priv->version_minor > 0) {
		...
		version = g_strdup_printf("%u.%u", priv->version_major, priv->version_minor);
...
static void
jcat_file_export_builder(JcatFile *self, JsonBuilder *builder, JcatExportFlags flags)
{
	JcatFilePrivate *priv = GET_PRIVATE(self);
		      ...
		      GError **error)
{
	...
	ostream = G_OUTPUT_STREAM(
	    g_file_replace(gfile, NULL, FALSE, G_FILE_CREATE_NONE, cancellable, error));
	if (ostream == NULL)
		return FALSE;
...
jcat_file_get_item_default(JcatFile *self, GError **error)
{
	...
	/* sanity check */
	if (priv->items->len == 0) {
		g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, "no items found");
		return NULL;
	}
	if (priv->items->len > 1) {
		g_set_error_literal(error,
