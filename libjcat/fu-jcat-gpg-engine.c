...
#include "fu-jcat-gpg-engine.h"

struct _FuJcatGpgEngine {
	FuJcatEngine parent_instance;
	gpgme_ctx_t ctx;
};

G_DEFINE_TYPE(FuJcatGpgEngine, fu_jcat_gpg_engine, FU_TYPE_JCAT_ENGINE)

G_DEFINE_AUTO_CLEANUP_FREE_FUNC(gpgme_data_t, gpgme_data_release, NULL)

static gboolean
fu_jcat_gpg_engine_add_public_key(FuJcatEngine *engine, const gchar *filename, GError **error)
{
	FuJcatGpgEngine *self = FWUPD_JCAT_GPG_ENGINE(engine);
	gpgme_error_t rc;
...
static gboolean
fu_jcat_gpg_engine_setup(FuJcatEngine *engine, GError **error)
{
	FuJcatGpgEngine *self = FWUPD_JCAT_GPG_ENGINE(engine);
	gpgme_error_t rc;
	...
	gpg_home = g_build_filename(fu_jcat_engine_get_keyring_path(engine), "gnupg", NULL);
...
static gboolean
fu_jcat_gpg_engine_check_signature(gpgme_signature_t signature, GError **error)
{
	gboolean ret = FALSE;
...
static FuJcatResult *
fu_jcat_gpg_engine_pubkey_verify(FuJcatEngine *engine,
				 GBytes *blob,
				 ...
				 GError **error)
{
	...
	for (s = result->signatures; s != NULL; s = s->next) {
		...
		if (!fu_jcat_gpg_engine_check_signature(s, error))
			return NULL;
...
static void
fu_jcat_gpg_engine_finalize(GObject *object)
{
	FuJcatGpgEngine *self = FWUPD_JCAT_GPG_ENGINE(object);
	if (self->ctx != NULL)
		gpgme_release(self->ctx);
	G_OBJECT_CLASS(fu_jcat_gpg_engine_parent_class)->finalize(object);
}

static void
fu_jcat_gpg_engine_class_init(FuJcatGpgEngineClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuJcatEngineClass *engine_class = FWUPD_JCAT_ENGINE_CLASS(klass);
	engine_class->setup = fu_jcat_gpg_engine_setup;
	engine_class->add_public_key = fu_jcat_gpg_engine_add_public_key;
	engine_class->pubkey_verify = fu_jcat_gpg_engine_pubkey_verify;
	object_class->finalize = fu_jcat_gpg_engine_finalize;
}

static void
fu_jcat_gpg_engine_init(FuJcatGpgEngine *self)
{
}

FuJcatEngine *
fu_jcat_gpg_engine_new(FuJcatContext *context)
{
	g_return_val_if_fail(FWUPD_JCAT_IS_CONTEXT(context), NULL);
	return FWUPD_JCAT_ENGINE(g_object_new(FU_TYPE_JCAT_GPG_ENGINE,
					      "context",
					      context,
					      "kind",
					      FWUPD_JCAT_BLOB_KIND_GPG,
					      "method",
