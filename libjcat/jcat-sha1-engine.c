...
static JcatBlob *
jcat_sha1_engine_self_sign(JcatEngine *engine, GBytes *data, JcatSignFlags flags, GError **error)
{
			     ...
			     GError **error)
{
	...
	if (g_bytes_compare(data_tmp, blob_signature) != 0) {
		g_autofree gchar *tmp2 = g_strndup(g_bytes_get_data(blob_signature, NULL), g_bytes_get_size(blob_signature));
		g_set_error(error,
			    G_IO_ERROR,
