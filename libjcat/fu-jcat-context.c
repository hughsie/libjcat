...
static void
fu_jcat_context_finalize(GObject *json_obj)
{
	FuJcatContext *self = FWUPD_JCAT_CONTEXT(json_obj);
...
fu_jcat_context_init(FuJcatContext *self)
{
	...
	g_ptr_array_add(priv->engines, fu_jcat_sha1_engine_new(self));
	g_ptr_array_add(priv->engines, fu_jcat_sha256_engine_new(self));
	g_ptr_array_add(priv->engines, fu_jcat_sha512_engine_new(self));
#ifdef ENABLE_GPG
	g_ptr_array_add(priv->engines, fu_jcat_gpg_engine_new(self));
#endif
#ifdef HAVE_GNUTLS_PKCS7
	g_ptr_array_add(priv->engines, fu_jcat_gnutls_pkcs7_engine_new(self));
#endif
#ifdef HAVE_LIBCRYPTO_PKCS7
	g_ptr_array_add(priv->engines, fwupd_jcat_libcrypto_pkcs7_engine_new(self));
#endif
#ifdef HAVE_GNUTLS_ED25519
	g_ptr_array_add(priv->engines, fu_jcat_gnutls_ed25519_engine_new(self));
#endif
}
...
{
	FuJcatContextPrivate *priv = GET_PRIVATE(self);
	g_return_if_fail(FWUPD_JCAT_IS_CONTEXT(self));
...
{
	FuJcatContextPrivate *priv = GET_PRIVATE(self);
	g_return_val_if_fail(FWUPD_JCAT_IS_CONTEXT(self), NULL);
			    ...
			    GError **error)
{
	...
	g_return_val_if_fail(FWUPD_JCAT_IS_BLOB(blob), NULL);
			    ...
			    GError **error)
{
	...
	g_return_val_if_fail(FWUPD_JCAT_IS_CONTEXT(self), NULL);
	g_return_val_if_fail(data != NULL, NULL);
	g_return_val_if_fail(FWUPD_JCAT_IS_ITEM(item), NULL);
	...
	for (guint i = 0; i < blobs->len; i++) {
		...
		result_str = fwupd_jcat_result_to_string(result);
			      ...
			      GError **error)
{
	...
	g_return_val_if_fail(FWUPD_JCAT_IS_CONTEXT(self), NULL);
	g_return_val_if_fail(FWUPD_JCAT_IS_ITEM(item_target), NULL);
	g_return_val_if_fail(FWUPD_JCAT_IS_ITEM(item), NULL);
