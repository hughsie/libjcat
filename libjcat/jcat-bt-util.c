			  ...
			  GError **error)
{
	...
	g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Ed25519 support is not enabled");
	return FALSE;
#else
		    ...
		    GError **error)
{
	...
	*parsed_key_hash = g_new0(gchar, 1 + key_hash_size / 2);
 ...
 * @error: #GError, or %NULL
 *
 * Parse a private key in the same format as Go's sumdb.
 * This works with the output of jcat_bt_generate_key_pair().
 *
 * Since: 0.2.2
 **/
gboolean
			  ...
			  GError **error)
{
	...
	*parsed_key_hash = NULL;
	...
	jcat_set_byte_array(parsed_private_key, decoded_key_content);
	jcat_set_byte_array(parsed_public_key,
 ...
 * @error: #GError, or %NULL
 *
 * Parse a public key in the same format as Go's sumdb.
 * This works with the output of jcat_bt_generate_key_pair().
 *
 * Note that if the private key is already being parsed, it is not necessary to parse the public
 * key afterwards since the public key can be found from the private key.
 *
 * Since: 0.2.2
 **/
gboolean
			 ...
			 GError **error)
{
	...
	/* like this: "test+69fd7e09+AXiHgzPfdDlQURC7UegWXKUFdjir/GC7uv45fTMEk5IN" */
	g_return_val_if_fail(parsed_public_key != NULL, FALSE);
	g_return_val_if_fail(parsed_key_name != NULL, FALSE);
	g_return_val_if_fail(parsed_key_hash != NULL, FALSE);
	...
	*parsed_key_hash = NULL;
	...
	/* next field is the key name */
	if (!parse_key_name(&data, &size, parsed_key_name, error))
		return FALSE;

	/* next field is the key hash */
	if (!parse_key_hash(&data, &size, &key_hash, error))
		return FALSE;

	/* final field is the base64-encoded key content -- needs to be NULL-terminated */
	decoded_key_content = parse_key_content(data, size, error);
	if (decoded_key_content == NULL)
		return FALSE;

	jcat_set_byte_array(parsed_public_key, decoded_key_content);
		  ...
		  GError **error)
{
	...
	*original_text = g_bytes_new(data, signature_start + 1 - data);
	 ...
	 *
	 * Note that we currently only support one, despite that in the canonical source the actual
	 * limit is (somewhat arbitrarily) 100.
	 *
	 * See
...
format_checkpoint(const gchar *cp_origin, guint64 cp_size, GBytes *cp_hash)
{
	...
	encoded_hash = g_base64_encode(g_bytes_get_data(cp_hash, NULL), g_bytes_get_size(cp_hash));
...
parse_formatted_checkpoint(GBytes *formatted_checkpoint,
			   ...
			   GError **error)
{
	/* parses the content formatted by the above function `format_checkpoint` */
	gsize size = 0;
	...
	*cp_size = g_ascii_strtoull(data, &end2, 10);
	...
	for (const gchar *p = data; p < end; ++p) {
		if (!g_ascii_isalnum(*p) && *p != '+' && *p != '/' && *p != '=') {
			g_set_error_literal(error,
					    G_IO_ERROR,
					    G_IO_ERROR_FAILED,
		       ...
		       GError **error)
{
	...
	if (!create_storage(storage_dir, error)) {
		g_prefix_error(error, "failed to create directories for storage: ");
		return FALSE;
	}
	if (!jcat_bt_parse_private_key(private_key_contents,
				       &parsed_private_key,
				       &parsed_public_key,
				       &parsed_key_name,
				       &parsed_key_hash,
	...
	if (!g_file_set_contents_full(checkpoint_file_path,
 ...
 * @error: #GError, or %NULL
 *
 * Parse a checkpoint that was read from the on-disk storage directory.
 * It requires that a valid log structure be found, the checkpoint be formatted correctly,
 * and the log origin be expected. If not, an empty checkpoint will be returned.
 *
 * Since: 0.2.2
 **/
gboolean
			 ...
			 GError **error)
{
	...
	if (g_strcmp0(cp_origin, expected_origin) != 0) {
		g_set_error(error,
			    G_IO_ERROR,
			    G_IO_ERROR_FAILED,
 ...
 * @assigned_seq: (not nullable) (out): sequence number
 * @error: #GError, or %NULL
 *
		    ...
		    GError **error)
{
	...
	g_return_val_if_fail(assigned_seq != NULL, FALSE);
	...
	if (original_seq != NULL) {
		/* parse and return the original sequence number -- unfortunately GBytes does not
		 * guarantee NULL termination but g_ascii_strtoull() requires it. */
		g_autofree gchar *original_seq_str =
		    g_strndup(g_bytes_get_data(original_seq, NULL), g_bytes_get_size(original_seq));
		gchar *end = NULL;

		errno = 0;
		*assigned_seq = g_ascii_strtoull(original_seq_str, &end, 16);
	...
	for (;;) {
		 ...
		 * existing file. */
		if (!g_file_copy(pending_leaf_file,
				 seq_file,
				 G_FILE_COPY_NONE, /* not G_FILE_COPY_OVERWRITE */
				 NULL,
				 NULL,
				 NULL,
				 &error_local)) {
		...
		seq_bytes = g_string_free_to_bytes(g_steal_pointer(&seq_str));
