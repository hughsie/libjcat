 ...
 */
static GByteArray *
jcat_bt_verified_prefix_hash_from_inclusion_proof(gint64 subSize,
						  gint64 size,
						  GPtrArray *proof,
						  GByteArray *root,
...
jcat_bt_generate_inclusion_proofs(void)
{
	...
	JcatBtInclusionProofTestVector *iptv = NULL;
		       ...
		       GError **error)
{
	...
	{
		g_autoptr(GPtrArray) probes = jcat_bt_generate_corrupt_inclusion_proof(leaf_index,
										       tree_size,
										       proof,
										       root,
										       leaf_hash);
		for (guint i = 0; i < probes->len; ++i) {
			...
			g_debug("testing jcat_bt_generate_corrupt_inclusion_proof index %u", i);
			if (jcat_bt_inclusion_proof_verify(p->leaf_index,
				...
				g_set_error(error,
					    G_IO_ERROR,
					    G_IO_ERROR_FAILED,
					    "jcat_bt_verifier_check case %s: incorrectly verified",
					    p->desc);
				return FALSE;
			}
			g_clear_error(error);
				   ...
				   GError **error)
{
	...
	{
		g_autoptr(GPtrArray) probes = jcat_bt_generate_corrupt_consistency_proof(snapshot1,
											 snapshot2,
											 root1,
											 root2,
											 proof);
...
jcat_bt_verify_inclusion_proof_func(void)
{
	...
	for (guint i = 0; i < 4; ++i) {
		...
		g_autoptr(GByteArray) emptyTreeHash =
	...
	for (guint i = 1; i < 6; ++i) {
		...
		g_autoptr(GError) error = NULL;
		g_debug("/proof:%u", i);
		jcat_bt_verifier_check(p->leaf - 1,
				       p->snapshot,
				       p->proof,
				       g_bytes_unref_to_array(root),
				       leaf_hash,
				       &error);
		g_prefix_error(&error, "jcat_bt_verifier_check() i = %u ", i);
		g_assert_no_error(error);
	}
}
...
{
	JcatBtInmemoryTree *tree = jcat_bt_inmemory_tree_create(0);
	...
	for (gsize k = 0; k < sizes->len; ++k) {
		...
		for (guint64 i = 0; i < size; ++i) {
			...
			g_prefix_error(&error,
				       "jcat_bt_verifier_check() i = %lu size = %lu ",
				       i,
				       size);
			g_assert_no_error(error);
		}
	}
	jcat_bt_inmemory_tree_free(tree);
}
...
jcat_bt_verify_consistency_proof_func(void)
{
	...
	g_autoptr(GPtrArray) proof2 =
	    g_ptr_array_new_with_free_func((GDestroyNotify)g_byte_array_unref);
	g_autoptr(GByteArray) emptyTreeRoot =
	    g_bytes_unref_to_array(jcat_bt_generate_sha256_empty_tree_hash());
...
{
	gint64 size = 130;
	JcatBtInmemoryTree *tree = jcat_bt_inmemory_tree_create(size);
	...
	for (gsize k = 0; k < sizes->len; ++k) {
		...
		for (gint64 i = 1; i <= (gint64)size; ++i) {
			...
			pRoot = jcat_bt_verified_prefix_hash_from_inclusion_proof(i,
										  ...
										  &error);
			g_prefix_error(&error,
				       "jcat_bt_verified_prefix_hash_from_inclusion_proof size %lu "
				       "prefix %ld ",
				       size,
				       i);
			g_assert_no_error(error);
...
jcat_bt_parse_checkpoint_func(void)
{
	...
	g_assert_nonnull(cp_hash);
	g_assert_cmpint(g_bytes_get_size(cp_hash), ==, 32);

	/* Truncated file: nothing shorter works */
	for (i = 0; i < sizeof good_checkpoint; ++i) {
		g_bytes_unref(checkpoint);
		g_bytes_unref(cp_hash);
		cp_hash = NULL;
		checkpoint = g_bytes_new_static(good_checkpoint, i);
		ret = jcat_bt_parse_checkpoint(checkpoint,
					       public_key,
					       "thisorigin",
					       &cp_size,
