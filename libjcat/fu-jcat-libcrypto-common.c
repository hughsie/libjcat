...
#define FWUPD_JCAT_RSA_SIZE 3072
/* To indicate that a certificate has no well-defined expiration date:
 * https://www.rfc-editor.org/rfc/rfc5280#section-4.1.2.5 */
#define FWUPD_JCAT_X509_NOTAFTER_UNDEFINED "99991231235959Z"
...
gchar *
fwupd_jcat_libcrypto_get_errors(void)
{
...
X509 *
fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_pem(GBytes *blob, GError **error)
{
...
X509 *
fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_der(GBytes *blob, GError **error)
{
	g_autoptr(BIO) bio_blob = NULL;
	g_autoptr(X509) crt = NULL;
	gsize blob_size = g_bytes_get_size(blob);
...
EVP_PKEY *
fwupd_jcat_libcrypto_pkcs7_load_privkey_from_blob_pem(GBytes *blob, GError **error)
{
	gsize blob_size;
...
GBytes *
fwupd_jcat_libcrypto_pkcs7_create_private_key(GError **error)
{
	...
	if (EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, FWUPD_JCAT_RSA_SIZE) <= 0) {
		g_autofree gchar *error_str = fwupd_jcat_libcrypto_get_errors();
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
...
GBytes *
fwupd_jcat_libcrypto_pkcs7_create_client_certificate(EVP_PKEY *privkey, GError **error)
{
	...
	/* set expiration */
	if (!ASN1_TIME_set_string(X509_getm_notAfter(crt), FWUPD_JCAT_X509_NOTAFTER_UNDEFINED)) {
		g_autofree gchar *error_str = fwupd_jcat_libcrypto_get_errors();
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
...
gchar *
fwupd_jcat_libcrypto_x509_get_issuer_name(X509 *crt, GError **error)
{
