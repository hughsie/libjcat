...
struct _FwupdJcatLibcryptoPkcs7Engine {
	FuJcatEngine parent_instance;
	X509_STORE *trust_store;
};

G_DEFINE_TYPE(FwupdJcatLibcryptoPkcs7Engine, fwupd_jcat_libcrypto_pkcs7_engine, FU_TYPE_JCAT_ENGINE)

static gboolean
fwupd_jcat_libcrypto_pkcs7_engine_add_pubkey_x509(FwupdJcatLibcryptoPkcs7Engine *self,
						  X509 *crt,
						  GError **error)
{
	...
	if (!X509_STORE_add_cert(self->trust_store, crt)) {
		g_autofree gchar *error_str = fwupd_jcat_libcrypto_get_errors();
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
...
static gboolean
fwupd_jcat_libcrypto_pkcs7_engine_add_public_key_raw(FuJcatEngine *engine,
						     GBytes *blob,
						     GError **error)
{
	FwupdJcatLibcryptoPkcs7Engine *self = FWUPD_JCAT_LIBCRYPTO_PKCS7_ENGINE(engine);
	g_autoptr(X509) cert = NULL;

	cert = fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_pem(blob, error);
	if (cert == NULL)
		return FALSE;
	return fwupd_jcat_libcrypto_pkcs7_engine_add_pubkey_x509(self, cert, error);
}

static gboolean
fwupd_jcat_libcrypto_pkcs7_engine_add_public_key(FuJcatEngine *engine,
						 ...
						 GError **error)
{
	...
	if (g_str_has_suffix(filename, ".pem")) {
		...
		if (blob == NULL)
			return FALSE;
		crt = fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_pem(blob, error);
		if (crt == NULL)
			return FALSE;
		if (!fwupd_jcat_libcrypto_pkcs7_engine_add_pubkey_x509(self, crt, error))
			return FALSE;
		...
		if (blob == NULL)
			return FALSE;
		crt = fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_der(blob, error);
		if (crt == NULL)
			return FALSE;
		if (!fwupd_jcat_libcrypto_pkcs7_engine_add_pubkey_x509(self, crt, error))
			return FALSE;
...
static gboolean
fwupd_jcat_libcrypto_pkcs7_engine_setup(FuJcatEngine *engine, GError **error)
{
	FwupdJcatLibcryptoPkcs7Engine *self = FWUPD_JCAT_LIBCRYPTO_PKCS7_ENGINE(engine);
...
cms_verify_cb(int ok, X509_STORE_CTX *ctx)
{
	...
	if (ok == 0) {
		...
		g_autofree gchar *issuer_name =
		    fwupd_jcat_libcrypto_x509_get_issuer_name(crt, NULL);
...
static FuJcatResult *
fwupd_jcat_libcrypto_pkcs7_engine_verify(FuJcatEngine *engine,
					 GBytes *blob,
					 ...
					 GError **error)
{
	...
	/* success */
	return FWUPD_JCAT_RESULT(g_object_new(FU_TYPE_JCAT_RESULT,
					      "engine",
					      engine,
					      "timestamp",
					      timestamp_newest,
...
static FuJcatResult *
fwupd_jcat_libcrypto_pkcs7_engine_self_verify(FuJcatEngine *engine,
					      GBytes *blob,
					      GBytes *blob_signature,
					      FuJcatVerifyFlags flags,
					      GError **error)
{
	...
	if (cert_blob == NULL)
		return NULL;
	crt = fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_pem(cert_blob, error);
	if (crt == NULL)
		return NULL;

	return fwupd_jcat_libcrypto_pkcs7_engine_verify(engine,
							blob,
							blob_signature,
							crt,
							flags,
...
static FuJcatResult *
fwupd_jcat_libcrypto_pkcs7_engine_pubkey_verify(FuJcatEngine *engine,
						GBytes *blob,
						GBytes *blob_signature,
						FuJcatVerifyFlags flags,
						GError **error)
...
fwupd_jcat_libcrypto_pkcs7_engine_pubkey_sign(FuJcatEngine *engine,
					      GBytes *blob,
					      GBytes *cert,
					      GBytes *privkey,
					      ...
					      GError **error)
{
	...
	if (g_bytes_get_size(blob) == 0) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_ARGUMENT,
				    "nothing to do");
		return NULL;
	}

	/* load keys */
	key = fwupd_jcat_libcrypto_pkcs7_load_privkey_from_blob_pem(privkey, error);
	if (key == NULL)
		return NULL;
	crt = fwupd_jcat_libcrypto_pkcs7_load_crt_from_blob_pem(cert, error);
	if (crt == NULL)
		return NULL;
...
static FwupdJcatBlob *
fwupd_jcat_libcrypto_pkcs7_engine_self_sign(FuJcatEngine *engine,
					    GBytes *blob,
					    FuJcatSignFlags flags,
					    GError **error)
{
	...
	if (g_file_test(fn_privkey, G_FILE_TEST_EXISTS)) {
		...
		privkey = fwupd_jcat_libcrypto_pkcs7_create_private_key(error);
		if (privkey == NULL)
			return NULL;
	...
	if (g_file_test(fn_cert, G_FILE_TEST_EXISTS)) {
		...
		if (key == NULL)
			return NULL;
		cert = fwupd_jcat_libcrypto_pkcs7_create_client_certificate(key, error);
		if (cert == NULL)
			return NULL;
		if (!fu_path_mkdir_parent(fn_cert, error))
			return NULL;
		if (!fu_bytes_set_contents_full(fn_cert, cert, 0666, error))
			return NULL;
	}

	/* sign */
	return fwupd_jcat_libcrypto_pkcs7_engine_pubkey_sign(engine,
							     blob,
							     cert,
							     privkey,
							     flags,
...
static void
fwupd_jcat_libcrypto_pkcs7_engine_finalize(GObject *object)
{
	FwupdJcatLibcryptoPkcs7Engine *self = FWUPD_JCAT_LIBCRYPTO_PKCS7_ENGINE(object);

	X509_STORE_free(self->trust_store);

	G_OBJECT_CLASS(fwupd_jcat_libcrypto_pkcs7_engine_parent_class)->finalize(object);
}

static void
fwupd_jcat_libcrypto_pkcs7_engine_class_init(FwupdJcatLibcryptoPkcs7EngineClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuJcatEngineClass *engine_class = FWUPD_JCAT_ENGINE_CLASS(klass);
	engine_class->setup = fwupd_jcat_libcrypto_pkcs7_engine_setup;
	engine_class->add_public_key = fwupd_jcat_libcrypto_pkcs7_engine_add_public_key;
	engine_class->add_public_key_raw = fwupd_jcat_libcrypto_pkcs7_engine_add_public_key_raw;
	engine_class->pubkey_verify = fwupd_jcat_libcrypto_pkcs7_engine_pubkey_verify;
	engine_class->pubkey_sign = fwupd_jcat_libcrypto_pkcs7_engine_pubkey_sign;
	engine_class->self_verify = fwupd_jcat_libcrypto_pkcs7_engine_self_verify;
	engine_class->self_sign = fwupd_jcat_libcrypto_pkcs7_engine_self_sign;
	object_class->finalize = fwupd_jcat_libcrypto_pkcs7_engine_finalize;
}

static void
fwupd_jcat_libcrypto_pkcs7_engine_init(FwupdJcatLibcryptoPkcs7Engine *self)
{
}

FuJcatEngine *
fwupd_jcat_libcrypto_pkcs7_engine_new(FuJcatContext *context)
{
	g_return_val_if_fail(FWUPD_JCAT_IS_CONTEXT(context), NULL);
	return FWUPD_JCAT_ENGINE(g_object_new(FWUPD_JCAT_TYPE_PKCS7_ENGINE,
					      "context",
					      context,
					      "kind",
