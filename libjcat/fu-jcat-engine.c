...
G_DEFINE_TYPE_WITH_PRIVATE(FuJcatEngine, fu_jcat_engine, G_TYPE_OBJECT)
#define GET_PRIVATE(o) (fu_jcat_engine_get_instance_private(o))

enum { PROP_0, PROP_CONTEXT, PROP_KIND, PROP_METHOD, PROP_VERIFY_KIND, PROP_LAST };

/* private */
void
fu_jcat_engine_add_string(FuJcatEngine *self, guint idt, GString *str)
{
	...
	fwupd_codec_string_append(str,
				  idt + 1,
				  "VerifyKind",
				  fu_jcat_engine_method_to_string(priv->method));
}

/* private */
gchar *
fu_jcat_engine_to_string(FuJcatEngine *self)
{
	GString *str = g_string_new(NULL);
	fu_jcat_engine_add_string(self, 0, str);
...
{
	FuJcatEngineClass *klass = FWUPD_JCAT_ENGINE_GET_CLASS(self);
	FuJcatEnginePrivate *priv = GET_PRIVATE(self);
 ...
 * @cert: #GBytes
 * @privkey: #GBytes
 * @flags: #FuJcatSignFlags, e.g. %FU_JCAT_VERIFY_FLAG_ADD_TIMESTAMP
 * @error: #GError, or %NULL
 *
...
{
	FuJcatEngine *self = FWUPD_JCAT_ENGINE(object);
	FuJcatEnginePrivate *priv = GET_PRIVATE(self);
	switch (prop_id) {
	case PROP_CONTEXT:
...
fu_jcat_engine_class_init(FuJcatEngineClass *klass)
{
	...
	pspec = g_param_spec_uint("verify-kind",
				  NULL,
				  NULL,
				  FWUPD_JCAT_BLOB_METHOD_UNKNOWN,
				  FWUPD_JCAT_BLOB_METHOD_LAST,
				  FWUPD_JCAT_BLOB_METHOD_UNKNOWN,
				  G_PARAM_READABLE | G_PARAM_DEPRECATED | G_PARAM_STATIC_NAME);
